...
class SettingWindow:
    ...
    def settingWindow(self,root):
        ...
        self.volume=ttk.Label(self.setting,text="Bobox's Volume: ",borderwidth=0,font=('"Times New Roman"')).place(x=3,y=17)
...
class WhatsApp:
    ...
    def send(self):
        ...
        while(True):
            try:
                pywhatkit.sendwhatmsg("+250"+mobile_number,message,datetime.datetime.now().hour,datetime.datetime.now().minute+1)
                break
            except Exception:
                pass
        time.sleep(20)
...
class Weather:
    def show(self,scrollable_text):
        ...
        SR.scrollable_text_clearing()
        SR.speak("-----------------------------Weather Report of KIGALI City------------------------------")
